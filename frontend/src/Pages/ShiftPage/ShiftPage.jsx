...
import { useDefPayActs } from "@/Hooks/DefpayactHooks/useDefPayActs"; // ✅ Added this
...
const ShiftPage = () => {
  ...
  const { fetchAccounts } = useDefPayActs(); // ✅ Hook now available to pass down
  ...
  const handleSubmitAll = async (finalData = formData) => {
    try {
      console.log("Submitting shift object:", finalData.shift);
      const result = await submitShift({
        shift: finalData.shift,
        deferals: finalData.deferals,
        payments: finalData.payments,
        token: user?.token,
      });
      ...
      case 1:
        return (
          <DefPayForm
            ...
            onNext={nextStep}
            onBack={prevStep}
            fetchAccounts={fetchAccounts} // ✅ Pass hook to form
          />
        );
      case 2:
        return (
